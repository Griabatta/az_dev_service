...
import { OzonAnalyticsSellDTO } from "./ozon-analytics-sell.dto";


@Injectable()
export class OzonAnalyticsSellRepository {

    constructor(
        private readonly prismaService: PrismaService
    ) {}

    async upserOzonSellAnalyticssManyRecord(data: OzonAnalyticsSellDTO[]) {
            const BATCH_SIZE = 30;
            const batches: OzonAnalyticsSellDTO[][] = [];
            
            if (data.length < BATCH_SIZE) {
            try {
                await this.prismaService.$transaction(
                data.map(data => 
                    this.prismaService.analytics.upsert({
                    where: { dimensionsIdCreatedAt: data.dimensionsIdCreatedAt },
                    update: data,
                    create: data
                    })
                )
