...
@Injectable()
export class OzonProductsSellRepository {

    constructor(
        private readonly prismaService: PrismaService
    ) {}

    async upserOzonSellProductsManyRecord(data: OzonProductsSellDTO[]) {
                const BATCH_SIZE = 30;
                const batches: OzonProductsSellDTO[][] = [];
                
                if (data.length < BATCH_SIZE) {
                try {
                    await this.prismaService.$transaction(
                    data.map(data => 
                        this.prismaService.product_List.upsert({
