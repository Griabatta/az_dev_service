...
@Controller('wbApi/stock')
export class WbStockContoller {

    private readonly logger = new Logger(WbStockContoller.name);

    constructor(
        private readonly stockService: WbStockService
    ) {
    ...
    async UpdateForUser(@Res() res: Response, @Req() req: Request, @Next() next: NextFunction) {
        ...
        try {
            if (typeof tgId === 'string') {
                await this.stockService.import(tgId); 
